 ...
 * UPDATE WITH RECORD
 */
async function updateDatabaseWithRecord(Model) {
  ...
  const emailRecordsFileName =
    './records/records-via-records/email-ingestion-records.json'
  ...
  ** non-data messages. */
  ...
  const emailIngestRecordsFail = {
    ...
    dupPieces, nonDataPcs, troublePcs, reportsAdded,
  }
  ...
  ** successful nonempty update. */
  ...
  // UPDATING FROM RECORD : USING RECORD HERE  (bsn=business, dev=development)
  //const lastMsgsRecordsFileName = './records/last-msgs-list.json' //latest
  //const lastMsgsRecordsFileName = './records/last-msgs-list-dev-0.json'
  //const lastMsgsRecordsFileName = './records/last-msgs-list-dev-1.json'
  //const lastMsgsRecordsFileName = './records/last-msgs-list-bsn-0.json'
  const lastMsgsRecordsFileName = './records/last-msgs-list-bsn-1.json'
  try { await fs.promises.access(lastMsgsRecordsFileName) }
  catch (error) { throw new Error('No record to update database with.') }
  let gmailsToRetrieve = await getJSONobject(lastMsgsRecordsFileName)
  if (emailHandlerIsInReprocessMode) {
    gmailsToRetrieve =
      removeOtherMsgs(troublePcs, gmailsToRetrieve, isVerbose)
  }
  // Even more narrow examination:
  //gmailsToRetrieve = [gmailsToRetrieve[4]]
  const latestGmailsIDs = gmailsToRetrieve.map(msgObj => msgObj.gmailIDs)


  // NOT THIS: UPDATING FROM RECORD
  ...
  ** form of `.eml` files. */
  ...
  // }
  let messagesList = gmailsToRetrieve
  // messagesList = [
  //   {
  //     gmailIDs: {..},
  //     headers: {..},
  ...
  // Record.
  // const lastMsgsRecordsFileName = './records/last-msgs-list.json'
  ...
  const logFileName = './records/records-via-records/updates-stats.log'
...
function removeOtherMsgs(keepTheseArray, fromThisArray, isVerbose) {
  let newArray = []
  let keepMsgIDs = keepTheseArray.map(msgObj => msgObj.id)
  let keepMsgThreadIDs = keepTheseArray.map(msgObj => msgObj.threadId)
  const len = keepTheseArray.length
  let keepCount = 0
  for (let msgObj of fromThisArray) {
    if (
      keepMsgIDs.includes(msgObj.gmailIDs.id) &&
      keepMsgThreadIDs.includes(msgObj.gmailIDs.threadId)
    ) {
  ...
  if (isVerbose) {
    // print warning if count isn't correct
    if (keepCount !== len) {
      console.log('WARNING!: The number of messages being kept from the ' +
        'list of messages is not the same as the number of old ' +
        'non-data messages being reprocessed (as trouble messages).')
    }
  }
  return newArray
...
module.exports = updateDatabaseWithRecord